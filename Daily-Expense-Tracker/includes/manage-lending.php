...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css"
        integrity="sha512-+SyhYdFyoOSvQxk89F0KTIJzWCEBmZwq3s0we3sNQ8WYt0Nv/b0+HkcgCZtqwWZzTJwR95G6Hrx0mEnm7MfXg=="
    ...
    <section class="home-section">
        <nav>
            ...
            <div class="search-box">
                <input input type="text" id="search-input" class="form-control form-control-sm mx-2"
                    placeholder="Search...">
            ...
            $(document).ready(function() {
                ...
                $('#search-input').on('keyup', function() {
                    var value = $(this).val()
                        .toLowerCase(); // Get search keyword and convert to lowercase
                    var found = false;

                    if (value) { // If search input has value
                        $('table tbody tr').filter(
                            function() { // Filter table rows based on search keyword
                                var matches = $(this).text().toLowerCase().indexOf(value) > -1;
                                $(this).toggle(
                                    matches); // Show or hide table rows based on search keyword
...
?>
                                            ...
                                            $(document).ready(function() {
                                                ...
                                                $('#select-entries').val(
                                                    limit); // Set selected option based on current limit value

                                                // Handle onchange event of select element
                                                $('#select-entries').on('change', function() {
                                                    var limit = $(this)
                                                        .val(); // Get selected limit value
                                                    var urlParams = new URLSearchParams(window.location
                                                        .search); // Get current URL query parameters
                                                    urlParams.set('limit',
                                                        limit); // Set new limit value
                                                    window.location.href = window.location.pathname +
                                                        '?' + urlParams
                                                        .toString(); // Reload page with new query parameters
                                                });
                                            });
                                            </script>
                                                            ...
                                                            <td>
                                                                ...
                                                                <div class="dropdown">
                                                                    <button
                                                                        ...
                                                                        type="button"
                                                                        id="dropdownMenuButton<?php echo $row['id']; ?>"
                                                                        data-toggle="dropdown" aria-haspopup="true"
                                                                        aria-expanded="false">
                                                                        Action
                                                                    </button>
                                                                    <div class="dropdown-menu"
                                                                        aria-labelledby="dropdownMenuButton<?php echo $row['id']; ?>">
                                                                        <a class="dropdown-item edit-btn" href="#"
                                                                            data-toggle="modal"
                                                                            data-target="#editExpenseModal<?php echo $row['id']; ?>"
                                                                            data-expense-id="<?php echo $row['id']; ?>"><i
                                                                                class="fas fa-edit"></i> Edit</a>
                                                                        <a class="dropdown-item"
                                                                            href="manage-lending.php?delid=<?php echo $row['id']; ?>"><i
                                                                                class="fas fa-trash-alt"></i> Delete</a>
                                                                    </div>
                                                                </div>

                                                                <div class="modal fade"
                                                                    id="editExpenseModal<?php echo $row['id']; ?>"
                                                                    tabindex="-1"
                                                                    ...
                                                                    <div class="modal-dialog">
                                                                        <div class="modal-content">
                                                                            ...
                                                                            <div class="modal-body">
                                                                                <form action="update-lending.php"
                                                                                    method="POST">
                                                                                    <div class="form-group">
                                                                                        <label
                                                                                            for="edit-expense-name">Expense
                                                                                            Name:</label>
                                                                                        <input type="text"
                                                                                            class="form-control"
                                                                                    ...
                                                                                    <div class="form-group">
                                                                                        <label
                                                                                            for="edit-expense-date">Date
                                                                                            of Lending:</label>
                                                                                        <input type="date"
                                                                                            class="form-control"
                                                                                            id="edit-expense-date"
                                                                                            name="date_of_lending"
                                                                                            value="<?php echo $row['date_of_lending']; ?>">
                                                                                    </div>
                                                                                    <div class="form-group">
                                                                                        <label
                                                                                            for="edit-expense-status">Status:</label>
                                                                                        <select class="form-control"
                                                                                            ...
                                                                                            <option value="received"
                                                                                                <?php if($row['status'] == 'received') echo 'selected'; ?>>
                                                                                                Received</option>
                                                                                        </select>
                                                                                    </div>
                                                                                    <div class="form-group">
                                                                                        <label
                                                                                            for="edit-expense-amount">Expense
                                                                                            Amount:</label>
                                                                                        <input type="number"
                                                                                            class="form-control"
                                                                                    ...
                                                                                    <div class="form-group">
                                                                                        <label
                                                                                            for="edit-expense-description">Expense
                                                                                            Description:</label>
                            ...
                            ?>
                                            ...
                                            <ul class="pagination justify-content-end">
                                                <li class="page-item <?php if($page <= 1){ echo 'disabled'; } ?>">
                                                    <a class="page-link"
                                                        href="<?php if($page <= 1){ echo '#'; } else { echo "?page=".($page - 1)."&limit=$limit"; } ?>">Previous</a>
                                                </li>
                                                <?php for($i = 1; $i <= $total_pages; $i++): ?>
                                                <li class="page-item <?php if($page == $i) echo 'active'; ?>">
                                                    <a class="page-link"
                                                        href="<?php echo "?page=$i&limit=$limit"; ?>"><?php echo $i; ?></a>
                                                </li>
                                                <?php endfor; ?>
                                                <li
                                                    class="page-item <?php if($page >= $total_pages) echo 'disabled'; ?>">
                                                    <a class="page-link"
                                                        href="<?php if($page >= $total_pages) { echo '#'; } else { echo "?page=".($page + 1)."&limit=$limit"; } ?>">Next</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
